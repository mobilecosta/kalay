...
User Function xfat001a()
	...
	oBrowse:acolumns[7]:cReadVar := 'TMP_QTDETQ'
...
Static Function fLoadData()
	...
	BeginSql Alias _cAlias
        ...
        SELECT D2_COD, B1_DESC, B1_UM, B1_QE, D2_QUANT, D2_ITEM, B1_LM
          FROM %TABLE:SD2% SD2
	...
	While(!(_cAlias)->(EoF()))
		...
		TRB->TMP_QTDETQ  := INT((_cAlias)->D2_QUANT/(_cAlias)->B1_LM)
...
Static Function TratarImp()
	...
	BeginSql Alias "TMPETIQ"
        ...
        SELECT D2_DOC, D2_SERIE, D2_COD, D2_QUANT, D2_CLIENTE, D2_LOJA, B1_DESC, B1_UM, B1_QE, A1_NOME, F2_VOLUME1, F2_EMISSAO, F2_ESPECI1, COALESCE(A4_NOME,"") A4_NOME, D2_PEDIDO, A1_MUN,; 
...
Static Function MontarImp()
	...
	oPrinter:SayAlign(nRow, nCol, AllTrim(TMPETIQ->A1_NOME), oArial17, 370, 50,, 2, 0)

	nRow += 90
	oPrinter:Say( nRow, nCol,"Transp: ",oArial17N,,,)
	oPrinter:Say( nRow, nCol+52,AllTrim(TMPETIQ->A4_NOME),oArial17,,,)

	nRow += 22
	oPrinter:Say( nRow, nCol,"Nr NFe: ",oArial17N,,,)
	oPrinter:Say( nRow, nCol+52,AllTrim(cNrNota),oArial17,,,)

	nRow += 22
	oPrinter:Say( nRow, nCol,"Série NFe: ",oArial17N,,,)
	oPrinter:Say( nRow, nCol+68,AllTrim(cSerie),oArial17,,,)

	nRow += 22
	oPrinter:Say( nRow, nCol,"Data NFe: ",oArial17N,,,)
	oPrinter:Say( nRow, nCol+65,dToC(TMPETIQ->F2_EMISSAO),oArial17,,,)

	nRow += 22
	oPrinter:Say( nRow, nCol,"Volumes: ",oArial17N,,,)
	oPrinter:Say( nRow, nCol+65,cValToChar(TMPETIQ->F2_VOLUME1) + " " + AllTrim(TMPETIQ->F2_ESPECI1),oArial17,,,)
