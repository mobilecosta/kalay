...
Static Function Requi()
	...
	Local oAutoAPI as Object
	cUrl := "https://my-json-server.typicode.com/Tiago-msC/teste/acesso"
	aHeader := {}
	Response := ""
	lAutorizado := .F.
	...
	If oRest:Get(aHeader)
		Response := oRest:GetResult()
		lJSonDes := FWJSonDeserialize(Response,@oAutoAPI)

		If(!lJSonDes)
			FWAlertError("Ocorreu um erro ao obter a autorização","Erro")
			lAutorizado := .F.
		Else
			lAutorizado := oAutoAPI:Permitido
		EndIf
	Else
		FWAlertError("Ocorreu um erro ao obter a autorização!!","Erro")
        lAutorizado := .F.
...
User Function TiDev()
    ...
    If .T.
        ...
        EndIf
    Else
        FWAlertError("Usuário não autorizado a acessar a ferramenta","Erro")
    EndIf
